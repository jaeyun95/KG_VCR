...
parser.add_argument(
    '-answer_ckpt',
    dest='answer_ckpt',
    default='models/saves/flagship_answer/best.th',
...
parser.add_argument(
    '-rationale_ckpt',
    dest='rationale_ckpt',
    default='models/saves/flagship_rationale/best.th',
...
def _to_gpu(td):
    if NUM_GPUS > 1:
        return td
    for k in td:
        td[k] = {k2: v.cuda(async=True) for k2, v in td[k].items()} if isinstance(td[k], dict) else td[k].cuda(async=True)
    return td
