...
class AttentionQA(Model):
    ...
    def forward(self,
        ...
        #print('qq2 ',q_rep.view(q_rep.shape[0], q_rep.shape[1], q_rep.shape[2]*q_rep.shape[3]).shape)
        padd_pre_qq_rep = torch.zeros(q_rep.shape[0], q_rep.shape[1], 100, q_rep.shape[3]).cuda()
        padd_pre_qq_rep[:,:,:q_rep.shape[2],:] = q_rep
        q_final = q_rep.view(q_rep.shape[0], q_rep.shape[1], q_rep.shape[2]*q_rep.shape[3])
        final_q_rep_pool = self.final_node_pool(q_final)

        final_material_rep = torch.cat((pooled_rep,final_q_rep_pool),-1)
