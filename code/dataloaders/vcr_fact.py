...
db = client.new_dataset
fact_part1 = db.fact_part1
##############################################################
...
##############################################################
## add new function for fact base
def _fix_matching_word(tokenized_sent, tokenized_number,  bert_embs, pad_ind=-1):
    """
    Matching word
    :param tokenized_sent: Tokenized sentence with detections collapsed to a list.
    :return: tokenized sentence
    ...
    for tok in tokenized_sent:
        if '_' in tok:
            tok.replace('_',' ')
            #bert embedding vector matching
    text_field = BertField([Token(x[0]) for x in new_tokenization_with_tags],
                           bert_embs,
                           padding_value=0)
    tags = SequenceLabelField([x[1] for x in new_tokenization_with_tags], text_field)
    ...
    return text_field, tags
##############################################################
def _fix_tokenization_fact(tokenized_sent, bert_embs, old_det_to_new_ind, obj_to_type, token_indexers, pad_ind=-1):
    """
    Turn a detection list into what we want: some text, as well as some tags.
    :param tokenized_sent: Tokenized sentence with detections collapsed to a list.
    :param old_det_to_new_ind: Mapping of the old ID -> new ID (which will be used as the tag)
...
##############################################################
## add fact embs
def _fix_matching_fact(fact_list, fact_embs):
    """
    Matching word
    :param tokenized_sent: Tokenized sentence with detections collapsed to a list.
    :return: tokenized sentence
    ...
    for facts in fact_list:
        pre_fact_list = []
        for fact in facts:
            with h5py.File(fact_embs, 'r') as h5:
                grp_fact_items = {k: np.array(v, dtype=np.float16) for k, v in h5[str(fact['fact_id'])].items()}
            fact['bert'] = grp_fact_items[f'fact']
            pre_fact_list.append(fact)
        final_fact_list.append(pre_fact_list)
    return final_fact_list
##############################################################

##############################################################
# bert_da
# add fact-hop parameter
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        if split not in ('test', '0810_train_scene_version', '0810_val_scene_version'):
        ...
        # if fact_hop == None --> 1
        self.fact1 = os.path.join(VCR_ANNOTS_DIR, f'{self.embs_to_load}_fact_part1.h5')
        #self.fact2 = os.path.join(VCR_ANNOTS_DIR, f'{self.embs_to_load}_fact_part2.h5')
        #self.fact3 = os.path.join(VCR_ANNOTS_DIR, f'{self.embs_to_load}_fact_part3.h5')



    @property
    def is_train(self):
        return self.split == '0810_train_scene_version'
    ...
    def splits(cls, **kwargs):
        ...
        train = cls(split='0810_train_scene_version', **kwargs_copy)
        val = cls(split='0810_val_scene_version', **kwargs_copy)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'rationale':
            ...
            # add for fact
            ###################################################################
            item['question_keyword'] += item['answer_keyword'][conditioned_label]
            item['question_keyword_num'] += item['answer_keyword_num'][conditioned_label]
            ###################################################################
        answer_choices = item['{}_choices'.format(self.mode)]
        dets2use, old_det_to_new_ind = self._get_dets_to_use(item)
        # add for fact
        ###################################################################
        answer_keyword = item['{}_keyword'.format(self.mode)]
        answer_keyword_num = item['{}_keyword_num'.format(self.mode)]
        ###################################################################
        with h5py.File(self.h5fn, 'r') as h5:
        ...
        if 'endingonly' not in self.embs_to_load:
            ...
            question_embs = questions_tokenized[0].embs
            #print('aa : ',question_embs.shape)
        #print('size : ', type(question_embs.shape[0]))
        ...
        fact_keyword_list = []
        for k in range(0,4):
            fact_keyword_list.append(item['visual_concept'][:])
            fact_keyword_list[k] = fact_keyword_list[k] + item['question_keyword'][:]
            fact_keyword_list[k] = list(set(fact_keyword_list[k]))


        for j,keywords in enumerate(answer_keyword):
            for keyword in keywords:
                if keyword.lower() not in fact_keyword_list[j]:
                    fact_keyword_list[j].append(keyword.lower())
        ###################################################################
        ########### Extract Knowledge ###########
        ...
        start = time.time()
        start = time.time()
        for j,ans_keys in enumerate(answer_keyword):
            pre_fact_list = []
            sub_ans_keys = ans_keys[:]
            for k,ans_key in enumerate(ans_keys):
                sub_ans_keys.remove(ans_key)
                #print('before : ', sub_ans_keys)
                #print('key : ', ans_key)
                for sub_ans_key in sub_ans_keys:
                    cursor = fact_part1.find({"$or": [{"e1":{"$regex":ans_key},"e2":{"$regex":sub_ans_key}},{"e1":{"$regex":sub_ans_key},"e2":{"$regex":ans_key}}]},{'_id':0,'word':0})
                    pre_fact_list += list(cursor)
                cursor_an = fact_part1.find({"$or": [{"e1": {"$regex": ans_key}}, {"e2": {"$regex": ans_key}}]},{'_id': 0, 'word': 0}).limit(15)
                pre_fact_list += list(cursor_an)
            for vc in item['visual_concept']:
                cursor_vc = fact_part1.find({"$or": [{"e1": {"$regex": vc}},{"e2": {"$regex": vc}}]},{'_id': 0, 'word': 0}).limit(15)
                pre_fact_list += list(cursor_vc)
            #ans_key
            fact_list.append(pre_fact_list)
            #print('j : ',j,' , len : ',len(pre_fact_list))

        #preparing fact embs
        #start = time.time()
        final_fact_list = _fix_matching_fact(fact_list,self.fact1)
        print('time : ',time.time()-start)
        #print(final_fact_list)
        ###################################################################
        ########### Scoring Fact ###########
        ##mk 31 size vector
        '''
        target = torch.zeros(31,768)
        source = torch.ones(5,768)
        target[:5,:] = source
        '''
        '''
        #device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        #torch.cuda.device(device)
        if question_embs.shape[0] != 31:
            #print('size : ',question_embs.shape[0])
            #padded_question_embs = np.zeros((31,768))
            padded_question_embs = np.zeros((31, 768))
            padded_question_embs[:question_embs.shape[0],:] = question_embs
            #print(padded_question_embs.shape)
        #print('here~~222')
        for final_fact in final_fact_list:
            for fact in final_fact:
                padded_final_fact = np.zeros((31,768))
                #print('dddddddddddddddddddd',final_fact)
                padded_final_fact[:fact['bert'].shape[0],:] = fact['bert']
                fact['bert'] = padded_final_fact
                #print(padded_final_fact.shape)
        #for
        #if len(final_fact_list) > 50:
        '''
        ...
        #instance_dict['final_fact_bert'] = ListField(final_fact_list)
        #instance_dict['final_fact_list'] = ListField(final_fact_list)
