...
def _to_gpu(td):
    if NUM_GPUS > 1:
        return td
    for k in td:
        if k != 'metadata' and k != 'graph_data':
...
loader_params = {'batch_size': 48 // NUM_GPUS, 'num_gpus':NUM_GPUS, 'num_workers':num_workers}

# edit vcr class for input model
...
#for epoch_num in range(start_epoch, params['trainer']['num_epochs']):
    train_results = []
    norms = []
    model.train()
    #batch size --> 100
